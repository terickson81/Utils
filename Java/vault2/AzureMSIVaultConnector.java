    ...
    public void retrieveSecrets() {
        try {
            ...
            if (response.isSuccessful()) {
                String accessToken = response.body().string();  // Parse the access token from the response.

                final VaultConfig config = new VaultConfig().address(VAULT_ADDR).build();
                final Vault vault = new Vault(config);

                AuthResponse authResponse = vault.auth().loginByAzure(VAULT_ROLE, accessToken);
